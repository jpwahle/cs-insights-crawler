...
from csinsights.s2orc import Paper
from csinsights.utils.citation_util import (
    BRACKET_REGEX,
...
from csinsights.utils.grobid_util import (
    extract_paper_metadata_from_grobid_xml,
    parse_bib_entry,
)
from csinsights.utils.refspan_util import sub_spans_and_update_indices
        ...
        try:
            ...
            if rtag.get("target"):
                ...
                if bracket:
                    # valid bracket span
                    if surface_span and (
                        surface_span[0] == "[" or surface_span[-1] == "]" or surface_span[-1] == ","
                    ):
                        pass
    ...
    if sp.abstract:
        ...
        # process all paragraphs
            ...
            for para in sp.abstract.find_all("p"):
                if para.text:
                    abstract_text.append(
                        process_paragraph(
                            sp, para, [(None, "Abstract")], bib_dict, ref_dict, cleanup_bracket
                        )
                    )
        # else just try to get the text
            ...
            if sp.abstract.text:
                abstract_text.append(
                    process_paragraph(
                        sp, sp.abstract, [(None, "Abstract")], bib_dict, ref_dict, cleanup_bracket
                    )
                )
