...
from typing import Any, BinaryIO, Dict, List, Optional, Set, Tuple, TypeVar
...
from nlpland.types import AccessType, DatasetJsonDict, FilterFunction, Url
...
def filter_by_access_fn(tree: DatasetJsonDict, access_types: Set[AccessType]) -> DatasetJsonDict:
    ...
    filtered = {
        element_list_key: [
            el
            for el in element_list
            if el.get("ee", {"@type": AccessType.OPEN}).get("@type")
            in access_types  # assume open access for all pubs that have no flag
        ]
        for element_list_key, element_list in tree.items()
    }
    return filtered
    ...
    def download_and_filter_release(
        ...
        **kwargs: Any,
        ...
        if dblp_use_filters:
            ...
            filters.extend([timestmap_filter])
