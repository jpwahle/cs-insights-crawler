...
class GrobidClient(ApiClient):
    ...
    def _output_file_name(self, input_file, input_path, output):
        ...
        if output is not None:
            ...
            filename = os.path.join(output, os.path.splitext(input_file_name)[0] + ".tei.xml")
        else:
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "path to the directory containing PDF files or .txt (for processCitationList only, one"
            " reference per line) to process"
        ),
    )
    parser.add_argument(
        "--output",
    ...
    if args.n is not None:
        try:
            ...
            print("Invalid concurrency parameter n:", args.n, ", n = 10 will be used by default")
            pass
